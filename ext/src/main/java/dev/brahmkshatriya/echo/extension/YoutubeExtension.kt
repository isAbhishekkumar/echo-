            ...
            video.streamingData.adaptiveFormats.forEach { format ->
                ...
                when (itag) {
                    ...
                    139, 140, 141, 249, 250, 251, 774 -> {
                        // --- FIX: Ensure qualityValue is Int ---
                        val qualityValue: Int = format.bitrate?.toInt() ?: when (itag) {
                    ...
                    // Fallback audio detection by mime type
                        ...
                        if (mimeType.contains("audio") && !mimeType.contains("video")) {
                            // --- FIX: Ensure qualityValue is Int ---
                            val qualityValue: Int = format.bitrate?.toInt() ?: 192000
                            val freshUrl = generateEnhancedUrl(originalUrl, 1, strategy, networkType)
                            val audioSource = createQualityAdaptiveSource(
                                freshUrl,
                                qualityValue,
                    ...
                    // Fallback video detection by mime type
                        ...
                        if (mimeType.contains("video") && !mimeType.contains("audio")) {
                            // --- FIX: Ensure qualityValue is Int ---
                            val qualityValue: Int = format.bitrate?.toInt() ?: 500000
                            val freshUrl = generateEnhancedUrl(originalUrl, 1, strategy, networkType)
                            val videoSource = createQualityAdaptiveSource(
                                freshUrl,
                                qualityValue,
                            ...
                            video.streamingData.adaptiveFormats.forEach { format ->
                                ...
                                when {
                                    isAudioFormat -> {
                                        ...
                                        val qualityValue: Int = when {
                                            format.bitrate != null && format.bitrate > 0 -> {
                                                val baseBitrate = format.bitrate.toInt() // Convert Long to Int
                                                when (networkType) {
                                            ...
                                            format.audioSampleRate != null -> {
                                                val sampleRate = format.audioSampleRate!!.toInt() // Convert Long to Int
                                                when (networkType) {
                                    ...
                                    isVideoFormat -> {
                                        ...
                                        if (showVideos) {
                                             // --- FIX: Ensure qualityValue is Int ---
                                            val qualityValue: Int = format.bitrate?.toInt() ?: 0 // Convert Long? to Int
                                            val freshUrl = generateEnhancedUrl(originalUrl, attempt, strategy, networkType)
                                    ...
                                    isCombinedFormat -> {
                                        ...
                                        if (showVideos) {
                                             // --- FIX: Ensure qualityValue is Int ---
                                            val qualityValue: Int = when {
                                                format.bitrate != null && format.bitrate > 0 -> format.bitrate.toInt() // Convert Long to Int
                                                format.height != null -> ((format.height!!.toLong() * 1000) + (format.bitrate?.toLong() ?: 0L)).toInt() // Convert Long to Int
